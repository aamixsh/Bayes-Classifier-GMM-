...
def calcPrereqTest(filename):
	...
	for n in range(N):
		file=open("../../data/Output/Clustering/Dataset 2/C/featureVectorsTest/features"+str(n+1)+".txt","w")
		imgSize=np.shape(np.array(tempData[n]))
		start_x=0
		for end_x in range(6,imgSize[0]):
			start_y,end_y=0,6
			tempMean=0.0
			tempVariance=0.0
			...
			# file.write()
			# file.write()
			for end_y in range(7,imgSize[1]):
				oldMean=tempMean
				...
				newMean/=49
				newVariance=oldVariance*49
				newVariance+=49*(newMean**2-oldMean**2)
				newVariance-=98*oldMean*(newMean-oldMean)
				for x in range(start_x,end_x+1):
			...
			# tempPatches.append(tempPatchX)
			start_x=start_x+1

def likelihood(x,uK,sigmaK):
	value=1/((2*math.pi)**(dimension/2))/((np.linalg.det(sigmaK))**0.5)
	temp=[0 for i in range(dimension)]
	mul=0
	sigmaInvK=np.asmatrix(sigmaK).I.A
	for i in range(dimension):
		for j in range(dimension):
			temp[i]+=(x[i]-uK[i])*sigmaInvK[j][i]
	for i in range(dimension):
		mul+=temp[i]*(x[i]-uK[i])
...
def calcPrereqTrain(direct):
	...
	#	Making patches and writing feature vectors to a file...
	file=open("../../data/Output/Clustering/Dataset 2/C/featureVectorsTrain/features.txt","w")
	for n in range(N):
		imgSize=np.shape(np.array(tempData[n]))
		start_x=0
		for end_x in range(6,imgSize[0]):
			...
			# tempPatches.append(tempPatchX)
			start_x=start_x+1
		# file.write(" ".join(str(Features) for Features in tempPatches[i][j]
...
print ("\nThis program clusters feature vectors of images into k-clusters.\n")
...
# calcPrereqTrain(direct)

if choice=='o':
	direct=raw_input("Enter the path (relative or complete) of the test data directory : ")	
else:
	direct="../../data/Input/Clustering/Dataset 2/C/test";

if direct[len(direct)-1]!='/':
	direct+="/";


calcPrereqTest(direct)
